...
if (newsCount.cnt === 0) {
  db.prepare(`
    ...
    (
      'Совет директоров холдинга одобрил расширение финансирования реального сектора',
      'Холдингтің Директорлар кеңесі нақты секторды қаржыландыруды кеңейтуді мақұлдады',
      ...
      '«Байтерек» холдингінің ҚР Премьер-Министрі Олжас Бектеновтің төрағалығымен өткен Директорлар кеңесі ЭКА-ға 10 жыл мерзімге 250 млрд теңге мемлекеттік кепілдік беруді мақұлдап, 2026–2028 жж. тұрақты даму жоспарын бекітті. 2026 жылдың басынан бері холдинг бизнеске 3 трлн теңгеге жуық қолдау көрсетті. 2025 жылдың қорытындысы бойынша Sustainable Fitch ESG-рейтингі «3»-тен «2»-ге көтерілді.',
      'Холдинг «Байтерек»',
      '/assets/news1.jpg'
    ),
    (
      'Фонд «Даму» запустил региональную программу «Іскер аймақ»',
      '«Даму» қоры «Іскер аймақ» өңірлік бағдарламасын іске қосты',
      'Фонд «Даму» запустил программу субсидирования малого бизнеса в регионах «Іскер аймақ». До 2028 года включительно программа предусматривает поддержку кредитов на 1,2 трлн тенге — около 5 тысяч проектов ежегодно. 30% бюджета направляется в приоритетные области, 70% — в остальные регионы, кроме Астаны и Алматы.',
      '«Даму» қоры өңірлерде шағын бизнесті субсидиялаудың «Іскер аймақ» бағдарламасын іске қосты. 2028 жылды қоса алғанда бағдарлама 1,2 трлн теңге несиені қолдауды көздейді — жыл сайын шамамен 5 мың жоба. Бюджеттің 30%-ы басым облыстарға, 70%-ы Астана мен Алматыдан басқа өңірлерге бағытталады.',
      'Даму',
      '/assets/news2.jpg'
    ),
    (
      'КЖК представила гарантии на реновацию жилья и программу «Орда Аймақ Плюс»',
      'ҚТҮК тұрғын үйді жаңғыртуға кепілдіктер мен «Орда Аймақ Плюс» бағдарламасын ұсынды',
      'На ежегодном форуме KHC Workshop-2026 в Астане Казахстанская жилищная компания представила новые инструменты: гарантии на реновацию ветхого жилья, ипотечную программу «Орда Аймақ Плюс» и обновления системы «Казреестр». Гарантийный портфель компании достиг 395 млрд тенге по более чем 50 проектам во всех регионах страны.',
      'Астанада өткен жыл сайынғы KHC Workshop-2026 форумында Қазақстандық тұрғын үй компаниясы жаңа құралдарды ұсынды: ескі тұрғын үйді жаңғыртуға кепілдіктер, «Орда Аймақ Плюс» ипотекалық бағдарламасы және «Қазтізілім» жүйесінің жаңартулары. Компанияның кепілдік портфелі елдің барлық өңірлеріндегі 50-ден астам жоба бойынша 395 млрд теңгеге жетті.',
      'КЖК',
      '/assets/news3.jpg'
    ),
    (
      'QIC запустил аграрный фонд прямых инвестиций Adal Fund для Центральной Азии',
      'QIC Орталық Азияға арналған Adal Fund тікелей инвестиция қорын іске қосты',
      ...
      'Qazaqstan Investment Corporation жиынтық капиталы шамамен 2,8 млрд доллар болатын 17 тікелей инвестиция қорына қатысады және Орталық Азия үшін нысаналы көлемі 1 млрд долларға дейінгі Adal Fund L.P. аграрлық қорын іске қосты. Салынған әр долларға QIC шетелдік серіктестерден шамамен 2,1 доллар тартады.',
      'QIC',
      '/assets/news4.jpg'
    ),
    (
      'ЭКА получит государственную гарантию на 250 млрд тенге для поддержки экспортёров',
      'ЭКА экспорттаушыларды қолдау үшін 250 млрд теңге мемлекеттік кепілдік алады',
      ...
      'Қазақстанның Экспорттық-кредиттік агенттігі (KazakhExport) 10 жыл мерзімге 250 млрд теңге мемлекеттік кепілдік алады. 2026 жылдың қорытындысы бойынша агенттіктің сақтандыру және кепілдік міндеттемелерінің портфелі 1,2 трлн теңгеге жетеді деп күтілуде, бұл қазақстандық шикізаттық емес өнім экспорттаушыларын қолдауды күшейтеді.',
      'ЭКА',
      '/assets/news5.jpg'
    )
  `).run();
...
app.get('/api/users', authenticate, (req, res) => {
  // Authors cannot see personal data — only basic info
  if (req.jwtUser?.role === 'author') {
    const rows = db.prepare('SELECT Id, FullName, Role, CreatedAt FROM Users').all();
...
app.patch('/api/users/:id/role', authenticate, (req, res) => {
  if (req.jwtUser?.role !== 'admin') {
    return res.status(403).json({ error: 'Доступ запрещён' });
  }
  const { role } = req.body;
  const allowedRoles = ['individual', 'author', 'admin'];
  if (!role || !allowedRoles.includes(role)) {
    return res.status(400).json({ error: 'Недопустимая роль' });
  }
  db.prepare('UPDATE Users SET Role = ? WHERE Id = ?').run(role, req.params.id);
  db.prepare('INSERT INTO AuditLogs (UserId, Action, EntityType, EntityId) VALUES (?, ?, ?, ?)')
    .run(req.jwtUser.id, 'UPDATE_USER_ROLE_' + role.toUpperCase(), 'User', req.params.id);
...
// ---- EXPORT ENDPOINTS (Task 6) ----
app.get('/api/export/applications', authenticate, (req, res) => {
  const rows = db.prepare(`
    SELECT a.Id, a.AppNumber, a.Status, a.CreatedAt, a.FormDataJson,
           s.Title as ServiceTitle, u.FullName as UserName, u.IIN as UserIin
    FROM Applications a
    LEFT JOIN Services s ON a.ServiceId = s.Id
    LEFT JOIN Users u ON a.UserId = u.Id
    ORDER BY a.CreatedAt DESC
...
app.get('/api/export/news', authenticate, (req, res) => {
  const rows = db.prepare('SELECT * FROM News ORDER BY CreatedAt DESC').all();
  res.json(rows);
});
...
app.post('/api/admin/login', (req, res) => {
  ...
  if ((login === 'admin' || login === 'admin@baiterek.kz') && password === 'baiterek2026') {
